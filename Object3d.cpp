...
void Object3d::Common::BasicInitGraphicsPiepeline(DirectXCommon* dxCommon)
{
	...
	ComPtr<ID3DBlob>normalColorPsBlob;//ピクセルシェーダオブジェクト

	//頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"shaders/BasicVS.hlsl",//シェーダファイル名
		nullptr,
	...
	//ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"shaders/BasicPS.hlsl",//シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,//インクルード可能にする
	...
	result = D3DCompileFromFile(
		L"shaders/BasicGS.hlsl",//シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,//インクルード可能にする
...
#pragma endregion
	result = dxCommon->GetDevice()->CreateGraphicsPipelineState(&gpipeline, IID_PPV_ARGS(&pipelinestate[0]));
	assert(SUCCEEDED(result));

	result = dxCommon->GetDevice()->CreateGraphicsPipelineState(&gpipeline2, IID_PPV_ARGS(&pipelinestate[1]));
	assert(SUCCEEDED(result));
}
