...
SceneObject::SceneObject()
{
	...
	stickUI = std::make_unique< Object3d>();
...
void SceneObject::Initialize(DirectXCommon* dxCommon, TextureManager* textureManager)
{
	...
	//Lスティックのモデルをセット
	stickUI->Initialize(modelLoader->GetModel(LSTICK));
	stickUI->SetScale(Vector3(3,3,3));
	stickUI->SetColor(Vector3(0.5f,0.5f,0.5f));
	stickUI->SetRotation(Vector3(0,180,0));
	//ボタンボードのモデルをセット
	buttonBorad->Initialize(modelLoader->GetModel(BOARD));
...
void SceneObject::PlayUpdate(Vector3 p, bool b)
{
	...
	hobCount += 0.2f;
	//Lスティックオブジェクト
	stickUI->SetPosition(p + Vector3(0, 5, 0));
	stickUI->SetRotation(Vector3(30, 180, sin(hobCount) * range));
	stickUI->Update();
...
{
	//Lスティックオブジェクト
	stickUI->Draw(dxCommon->GetCommandList());
