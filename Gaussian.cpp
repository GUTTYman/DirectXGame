...
const float Gaussian::clearColor[4] = { 0.9f,1.0f, 0.9f,1.0f };
...
void Gaussian::Initialize()
{
	...
	result = GetdxCommon()->GetDevice()->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), 	// アップロード可能
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer((sizeof(GaussianData) + 0xff) & ~0xff),
		D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(&cb));
	assert(SUCCEEDED(result));
	...
	cbvd.SizeInBytes = ((sizeof(GaussianData) + 0xff) & ~0xff);
...
void Gaussian::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	for (int i = 0; i < 2; i++)
	{
		//リソースバリアを変更
...
void Gaussian::PostDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	for (int i = 0; i < 2; i++)
	{
...
void Gaussian::CreateGraphicsPiepelineState()
{
	...
	result = D3DCompileFromFile(
		L"shaders/GaussianVS.hlsl",	// シェーダファイル名
		nullptr,
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"shaders/GaussianPS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
void Gaussian::Draw(ID3D12GraphicsCommandList* cmdList)
{

	GaussianData* peData = nullptr;
